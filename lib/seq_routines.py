...
if __name__ == '__main__':
    ...
    if rank == 0:
        ...
        class MuxExtractor(object):
            def __init__(self, g, read_dict, RefStart):
                self.g = g
                self.read_dict = read_dict
                self.RefStart = RefStart

            def __call__(self):
                log.debug("[MuxExtractor] - Reading channel: %s", self.g)
                fixed_dict = dict([(k, v) for k, v in self.read_dict.items() if len(v) > 0])  ## Remove empty keys
                ReadPerChannel = 0
                BasesPerChannel = 0
                FailedReads = 0
                MuxProductivity = {str(il): [] for il in range(1, 5)}
                for k, v in fixed_dict.iteritems():
                    v[:] = [v[z] for z in (y[0] for y in sorted(enumerate(zip(*v)[0]), key=lambda z: z[1]))]
                    TimeVec = map(lambda x: int(time.strftime('%H', time.localtime(float(x) - float(self.RefStart)))), zip(*v)[0])
                    for e in xrange(len(TimeVec)):
                        hour = TimeVec[e]
                        if e > 1:
                            if hour < TimeVec[e - 1]:
                                TimeVec[e] = hour + 24
                    MuxProd = {}
                    MuxBase = {}
                    v = zip(TimeVec, list(zip(*v)[1]), list(zip(*v)[2]))
                    ReadPerChannel += len(v)
                    BasesPerChannel += sum(map(int, zip(*v)[1]))
                    for hr, val, qual in v:
                        if float(qual) < 7.0:
                            FailedReads += 1
                        if str(hr) in MuxProd:
                            ...
                            MuxProd[str(hr)] = int(val)
                            MuxBase[str(hr)] = 1
                    MuxProductivity[str(k)].append(dict(zip(MuxBase.keys(), zip(MuxBase.values(), MuxProd.values()))))
                ObjectOut = []
                ObjectOut.append(str(self.g))
        ...
        def ChannelParser(ReadDict, RefStart, num_consumers):
            log.info('Start Channel Parser')
            BadDict = {'1': [], '2': [], '3': [], '4': []}
            ChannelDict = {str(il): () for il in range(1, 513)}
            Q1 = multiprocessing.JoinableQueue()
            R1 = multiprocessing.Queue()
            qdim = 0
            process_consumers_number = [Consumer(Q1, R1)
                                        for ix in xrange(num_consumers)]
            for w in process_consumers_number:
                w.start()
            for g in xrange(1, len(ReadDict) - 1):
                if ReadDict[str(g)] != BadDict:
                    qdim += 1
                    Q1.put(MuxExtractor(g, ReadDict[str(g)], RefStart))
                else:
                    log.warn('Channel %s inactive!' % (g))
            ...
            for r in xrange(qdim):
                result = R1.get()
                ChannelDict[str(result[0])] = result[1:5]

            log.info('Finish Channel Parser')
            return ChannelDict
            ...
            try:
                ...
                log.error(
                    "Can't find ID in html file!\n\033[36m##################################\n\033[93m%s\033[32m%s\033[93m%s\033[36m##################################\n\033[0m" % (
                        tb[0], tb[1], tb[2]))
                sys.exit(1)
            FixLine = "<script type = 'text/javascript'>var graph = document.getElementById('" + FileId + "');graph.on('plotly_hover', function(data){var divname = data.points.map(function(d){return (d.data.name)});if(divname == 'ReadsPerHour' || divname == 'BasesPerHour' || divname == 'MeanReadLengthPerHour'){if(data.xvals){Plotly.Fx.hover(graph, {xval:data.xvals[0] }, ['x5y', 'x5y3','x5y5']);}}});</script>"
        ...
        def FastStats(ChannelOut):
            ...
            for c in SmallDict:
                Sd = dict([(k, v) for k, v in ChannelOut[str(c)][3].items() if len(v) > 0])
                for k, v in Sd.iteritems():
                    for o, r in v[0].iteritems():
                        HoursDict[o][0] += r[0]
                        HoursDict[o][1] += r[1]

            PassSum = sum(map(lambda x: int(HoursDict[x][0]), HoursDict.keys()))
            for x in sort([int(k) for k, v in HoursDict.iteritems() if v != [0, 0, 0]]):
                HoursDict[str(x)][2] = str(round(float(HoursDict[str(x)][1]) / float(HoursDict[str(x)][0])))

            SmallDict2 = dict([(k, v) for k, v in read_dict.items() if v != {'1': [], '2': [], '3': [], '4': []}])
            MaxMinvec = []
            LengthsVec = [0]
            QualsVec = [0]
            GCVec = [0]
            for c in SmallDict2:
                Bait = map(int, [zip(*v)[1] for k, v in SmallDict2[str(c)].items() if len(v) > 0][0])
                LengthsVec.extend(Bait)
                QualsVec.extend(map(float, [zip(*v)[2] for k, v in SmallDict2[str(c)].items() if len(v) > 0][0]))
                GCVec.extend(map(float, [zip(*v)[3] for k, v in SmallDict2[str(c)].items() if len(v) > 0][0]))
            ...
            MeanReadLength = round(mean(map(float, map(lambda x: HoursDict[str(x)][2],
                                                       sort([int(k) for k, v in HoursDict.iteritems() if v != [0, 0]])))), 1)
    ...
    def mpi_caller(m_f, Norder, Fq_flag):
        import fast5_reader as f5r
        MinTimeset = set()
        rffqlist = []      
        ChrDMux = {str(el): {str(il): [] for il in range(1, 5)} for el in range(1, 513)}
        p_d, MinTime, readsforfq = f5r.parsing_func(m_f, Norder, tmp_dir, Fq_flag)
        MinTimeset.add(MinTime)
        for k, v in p_d.iteritems():
             if v is not {}:
                 for ch, l in v.iteritems():
                     ChrDMux[k][ch] = ChrDMux[k].get(ch, []) + l
        MinT = min(MinTimeset)
        if Fast_flag == True:
            rffqlist = rffqlist + readsforfq
            file_out = os.path.join(tmp_dir, 'tmp.' + str(random.randint(1,21000000)) + '.fastq.gz')
            Gzout = f5r.yielder(rffqlist)
            with bgzf.BgzfWriter(file_out, "wb") as outgz:
                SeqIO.write(sequences=Gzout, handle=outgz, format="fastq")            
        return ChrDMux, MinT, set(rffqlist)
    ...
    for job in enumerate(my_files):
        results.append(mpi_caller(job[1], job[0], Fast_flag))
    results = comm.gather(results, root=0)
    if rank == 0:
        results = [_i for temp in results for _i in temp]
        MinTimeset = set()
        read_dict = {str(el): {str(il): [] for il in range(1, 5)} for el in range(1, 513)}
        for res in results:
            p_d, MinTime, fastqout = res
            MinTimeset.add(MinTime)
            for k, v in p_d.iteritems():
                 if v is not {}:
                     for ch, l in v.iteritems():
                         read_dict[k][ch] = read_dict[k].get(ch, []) + l
        ref_start = min(MinTimeset)             
                                           
        if not os.path.exists(os.path.join(work_dir, out_dir)):
            os.makedirs(os.path.join(work_dir, out_dir))
        ChOut = ChannelParser(read_dict, ref_start, size)
        import plotly
        import plotly.graph_objs as go        
        result_plotting(ChOut)
        FastStats(ChOut)
        if Fast_flag == True:
